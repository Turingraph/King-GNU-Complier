...
8.	Display 3D input text when use `./fdf "Hello World"` base on FIGlet style ASCII files input (`text/a.txt`, `text/b.txt` etc.). If there is no target files, then return the error.
9.	`--Adir` option to specify which folder that have `dir/a.txt`, `dir/b.txt`, etc. to include for `./fdf "Goodbye Mars"` input.
10.	`--obj_rgb=#FF0000`, `--background_rgb=#000000`, and `--light_rgb=#FFFFFF` to specify the color of 3D object, background, and light, accordingly. The default color of objects, background, and light, are red, black, and white, accordingly.
11.	`synesthesia.txt` as 1 column text to color object based on user defined graphite color synesthesia as the default color of 3D object (instead of red in 10-th feature), is based on the color of the first character of user string/file name input. If `synesthesia.txt` file don't exists in the current directory, than the default color of the object is red.
