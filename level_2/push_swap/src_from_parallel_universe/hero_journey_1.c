	...
	size_t tree_of_life, char whoami)
{
	...
	// 4 * 2 % 6 = 8 % 6 = 2
	if (tree_of_life * day % life_energy == 2)
		if (original_time->first != NULL
			&& original_time->first->future != NULL
			&& original_time->first->moment > original_time->first->future->moment)
			arc_prioritize(original_time->first, original_time->first->future, whoami);
	// else if ()
	return (life_energy);
}
